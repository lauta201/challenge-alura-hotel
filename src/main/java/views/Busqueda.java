...
import com.hotel.jdbc.controller.UsuarioController;
...
public class Busqueda extends JFrame {
		...
		btnbuscar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (txtBuscar.getText().equals("")) {
					...
					llenarTablasReservaId();
					llenarTablasUsuarioId();
				}
			}
		});
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas != 0) {
					...
					if(confirmar == JOptionPane.YES_OPTION){
						...
						reservaController.eliminar(valor);
					...
					if(confirmarh == JOptionPane.YES_OPTION){
						...
						usuarioController.eliminar(valor);
						JOptionPane.showMessageDialog(contentPane, "Registro Eliminado");
						limpiarTabla();
						llenarTablasUsuario();
	...
	private List<Reserva> buscarReserva() throws SQLException {
	...
	private List<Usuario> buscarUsuarioId() throws SQLException {
		return this.usuarioController.buscarId(txtBuscar.getText());
	}

	private List<Usuario> buscarUsuarios() throws SQLException {
		return this.usuarioController.buscarUsuarios();
	}

	private void llenarTablasReservaId() {
		List<Reserva> reserva;
		reserva = buscarReservaId();
		for (Reserva reservas : reserva) {

			modelo.addRow(new Object[] { reservas.getId(), reservas.getFechaEntrada(), reservas.getFechaSalida(),
					reservas.getValorReserva(), reservas.getFormaPago() });
		}
	}

	private void llenarTablasReserva() {
		List<Reserva> reserva;
		try {
	...
	private void llenarTablasUsuarioId() {
		List<Usuario> usuario;
		try {
			usuario = buscarUsuarioId();

			for (Usuario usuarios : usuario) {

				modeloHuesped.addRow(new Object[] { usuarios.getId(), usuarios.getNombre(), usuarios.getApellido(),
						usuarios.getFechaNacimiento(), usuarios.getNAcionalidad(), usuarios.getTelefono(),
						usuarios.getIdReserva() });
			}
	...
	private void llenarTablasUsuario() {
		List<Usuario> usuario;
		try {
			...
			for (Usuario usuarios : usuario) {
				...
				modeloHuesped.addRow(new Object[] { usuarios.getId(), usuarios.getNombre(), usuarios.getApellido(),
						usuarios.getFechaNacimiento(), usuarios.getTelefono(), usuarios.getNAcionalidad(),
						usuarios.getIdReserva() });
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	private void actualizarReserva() {
        ...
        .ifPresentOrElse(fila -> {
			...
			this.reservaController.editar(fechaE, fechaS, valor, formaPago, id);
	...
	private void actualizarHuesped(){		
	        ...
	        .ifPresentOrElse(filaHuesped -> {
				...
				this.usuarioController.actualizarHuesped(nombre, apellido, fechaN, telefono, nacionalidad, idReserva, id);
				JOptionPane.showMessageDialog(this, String.format("Registro modificado con éxito"));
			},() -> JOptionPane.showMessageDialog(this, "Por favor, elije un registro"));}
